...
def ip_lookup_main():
    ip = simpledialog.askstring("ip", "Enter an IP adresse to scan : ")
    if not ip:
        text_area.insert("Please, enter an appropriate IP adress.")
        return
    ip_lookup(ip)

def Subdomains_Scanner_main():
    Subdomain = simpledialog.askstring("Subdomain","Enter a domain's name to scan : ")
    if not Subdomain:
        text_area.insert(END,"Please, enter an appropriate domain's name.")
        return
    Words_Length = simpledialog.askinteger("Word Length", "Enter the Word range you want to test in the word list (Max 114441) : ")
    if not Words_Length:
...
def Whois_Lookup_main():
    Domain = simpledialog.askstring("Enter a domain's name : ")
    if not Domain:
        text_area.insert(END,"Enter an appropriate domain's name.")
        return
